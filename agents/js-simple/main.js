      ...
      socket.send(JSON.stringify({
        ...
        elements: [
          ...
          { name: 's1', type: 'slider', disp: 'A slider',
            min: -1, max: 1, step: 0.01, val: value1 },
          { name: 's2', type: 'slider', disp: 'Another slider',
            min: 0, max: 10, step: 1, val: value2 },
        ]
      }))
    } else if (o.type === 'act') {
      if (o.action === 'a1') action1()
      if (o.action === 'a2') action2()
      socket.send(JSON.stringify({ type: 'done', ts: o.ts, action: o.action }))
    } else if (o.type === 'set') {
      if (o.key === 's1') value1 = o.val
      if (o.key === 's2') value2 = o.val
      socket.send(JSON.stringify({ type: 'upd', ts: o.ts, key: o.key, val: o.val }))
    }
  }
}
reconnect()
...
setInterval(() => {
  value2 = (value2 + 1) % 11
  if (socket !== undefined)
    socket.send(JSON.stringify({ type: 'upd', ts: '', key: 's2', val: value2 }))
}, 5000)
